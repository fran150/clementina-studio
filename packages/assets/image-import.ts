 ...
 * the library afterwards. `plane` is the 1bpp page being edited, which lives in
 * the editor rather than on the tileset.
 */
export type ResolvedTileset = Tileset & {palettes:number[];plane:number};
...
export interface ImageImportResult { tileset:ResolvedTileset;tilesWide:number;tilesHigh:number;overwrittenTiles:number[];
...
/** Weighted median-cut quantization; color index zero is reserved for transparency. */
...
/** Pure preview: never mutates the input tileset or image. Imports replace complete destination tiles. */
export function convertTilesetImage(original:ResolvedTileset,image:Raster,o:ImageImportOptions):ImageImportResult {
 ...
 if(!integer(o.tileX,0,16-tw)||!integer(o.tileY,0,16-th))throw Error('The imported tiles extend beyond this tileset. Choose another position or a smaller region.');
 ...
 const tileset=structuredClone(original),limit=tileset.bpp===1?1:7,targets=new Set<number>();
 for(let y=0;y<th;y++)for(let x=0;x<tw;x++)targets.add((o.tileY+y)*16+o.tileX+x);
 const reserved=new Set(o.protectedPalettes??[]);original.tilePaletteBanks.forEach((p,t)=>{if(!targets.has(t))reserved.add(p);});
 const result:ImageImportResult={tileset,tilesWide:tw,tilesHigh:th,overwrittenTiles:[],createdPalettes:[],remappedPixels:0,limitedTiles:0,transparentPixels:0};
 const planes=tileset.bpp===1?[tileset.plane]:[0,1,2];
 const nearest=(word:number,p:number)=>{let index=1,error=Infinity;for(let i=1;i<=limit;i++){const d=distance(word,tileset.palettes[p*8+i]);if(d<error){error=d;index=i;}}return {index,error};};
 ...
 for(let ty=0;ty<th;ty++)for(let tx=0;tx<tw;tx++){
  ...
  if(planes.some(p=>tileset.chr.slice(p*2048+t*8,p*2048+t*8+8).some(v=>v!==0)))result.overwrittenTiles.push(t);
  ...
  let palette=tileset.tilePaletteBanks[t];
  if(hist.size){let best=Infinity;for(let p=0;p<16;p++){const error=score(hist,p);if(error<best){palette=p;best=error;}}
   if(o.paletteMode==='create'&&best>0){const colors=quantize(hist,limit),ideal=[...hist].reduce((sum,[word,n])=>sum+Math.min(...colors.map(c=>distance(word,c)))*n,0);const free=best>ideal+.0001? Array.from({length:16},(_,i)=>i).find(p=>!reserved.has(p)):undefined;
    if(free!==undefined){palette=free;for(let i=1;i<=limit;i++)tileset.palettes[palette*8+i]=colors[(i-1)%colors.length];result.createdPalettes.push(palette);}
    else if(best>ideal+.0001)result.limitedTiles++;
   }
   reserved.add(palette);tileset.tilePaletteBanks[t]=palette;
  }
  ...
  words.forEach((word,i)=>{let value=0;if(word!==null){const found=nearest(word,palette);value=found.index;if(found.error>0)result.remappedPixels++;}
   for(const p of planes){const bit=tileset.bpp===1?(value?1:0):(value>>p)&1;tileset.chr[p*2048+t*8+Math.floor(i/8)]|=bit<<(i%8);}
  });
 }
 return result;
}
